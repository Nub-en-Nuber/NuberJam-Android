package com.example.nuberjam.ui.main.library.detail.deleteplaylist
...
class DeletePlaylistDialogFragment : DialogFragment() {
    ...
    private fun observeDeleteState() {
        viewLifecycleOwner.collectLifecycleFlow(viewModel.deletePlaylistState) { result ->
            showLoading(result is Result.Loading)
            when (result) {
                is Result.Success -> {
                    setFragmentResult(
                        DetailLibraryFragment.DELETE_PLAYLIST_REQUEST_KEY,
                        bundleOf(BundleKeys.DELETE_PLAYLIST_STATE_KEY to true)
                    )
                    dismiss()
                }
    ...
    private fun initLayout() {
        binding.apply {
            tvTitle.text = getString(R.string.title_delete_playlist)
            tvConfirmation.text = getString(R.string.playlist_delete_confirmation)
    ...
    companion object {
        const val TAG = "DeletePlaylistDialogFragment"

        fun getInstance(id: Int): DeletePlaylistDialogFragment =
            DeletePlaylistDialogFragment().apply {
                arguments = Bundle().apply {
                    putInt(BundleKeys.PLAYLIST_ID_KEY, id)
                }
            }
    }
}